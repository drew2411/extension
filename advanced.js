...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const newClassifyStatusEl = document.getElementById('newClassifyStatus');
    const classifySearchInput = document.getElementById('classifySearch');
    const categorizedWebsitesList = document.getElementById('categorizedWebsitesList');
    const suggestionsCard = document.getElementById('classificationSuggestionsCard');
    const suggestionsList = document.getElementById('classificationSuggestionsList');
    ...
    function renderClassifications(classifications) {
        ...
        if (!categorizedWebsitesList) return;

        const query = classifySearchInput?.value.toLowerCase().trim() || '';
        categorizedWebsitesList.innerHTML = '';
        ...
        if (filtered.length === 0) {
            categorizedWebsitesList.innerHTML = '<li class="empty-state">No categorized websites matching search.</li>';
            return;
        }
        ...
        filtered.forEach(([domain, category]) => {
            ...
            applyRemoveButtonStyles(delBtn);
            ...
            categorizedWebsitesList.appendChild(li);
        });
    }
    ...
    function renderSuggestionsList() {
        ...
        chrome.storage.local.get(['timeHistory', 'domainClassifications'], (res) => {
            ...
            suggestions.slice(0, 5).forEach(([domain, totalSecs]) => {
                ...
                textSpan.innerHTML = `<b>${domain}</b> &nbsp;<span style="color:#94a3b8;font-size:11px;">(${formatDuration(totalSecs)} this week)</span>`;
    ...
    if (addClassifyBtn) {
        addClassifyBtn.addEventListener('click', () => {
            const domain = newClassifyDomainInput.value.trim().toLowerCase().replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
            if (!domain) {
                showStatusMessage(newClassifyStatusEl, 'Domain is required.', true);
                return;
            }
            const category = newClassifyCategorySelect.value;
            chrome.storage.local.get(['domainClassifications'], (res) => {
                const list = res.domainClassifications || {};
                list[domain] = category;
                chrome.storage.local.set({ domainClassifications: list }, () => {
                    newClassifyDomainInput.value = '';
                    showStatusMessage(newClassifyStatusEl, 'Website added successfully!');
                });
            });
        });
    }

    if (classifySearchInput) {
        classifySearchInput.addEventListener('input', () => {
            renderClassifications(currentClassifications);
        });
    }

    function showStatusMessage(el, msg, isError = false) {
        if (!el) return;
        el.textContent = msg;
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
        ...
        if (changes.domainClassifications) {
            renderClassifications(changes.domainClassifications.newValue || {});
        }
